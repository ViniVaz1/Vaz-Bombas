...
let filtroPostosAtivo = 'todos';

function setFiltroPostos(filtro, el){
  filtroPostosAtivo = filtro;
  // Reset visual de todos os botões
  ['todos','vencido','hoje','semana','pago','sempag'].forEach(f=>{
    const btn = document.getElementById('ftab-'+f);
    if(!btn) return;
...
function renderPostos(){
  ...
  let filtrados=!termo?[...postos]:postos.filter(p=>
  ...
  if(filtroPostosAtivo !== 'todos'){
    filtrados = filtrados.filter(p => {
      const pagouMes = p.ultimoPagamento && p.ultimoPagamento.slice(0,7) === mesAtualGlobal;
      const sm = getStatusMensalidade(p);
      if(filtroPostosAtivo === 'sempag')  return !p.mensalidade;
      if(filtroPostosAtivo === 'pago')    return p.mensalidade && pagouMes;
      if(!sm || pagouMes) return false; // pagos ficam fora dos filtros de vencimento
      if(filtroPostosAtivo === 'vencido') return sm.diffMesAtual < 0;
      if(filtroPostosAtivo === 'hoje')    return sm.diffMesAtual === 0;
      if(filtroPostosAtivo === 'semana')  return sm.diffMesAtual > 0 && sm.diffMesAtual <= 7;
      return true;
    });
  }
  ...
  if(!filtrados.length){
    const msgs = {
      todos:   termo ? 'Nenhum posto encontrado para "<strong>'+esc(termo)+'</strong>"' : 'Nenhum posto cadastrado.',
      vencido: 'Nenhum posto com mensalidade vencida. ✅',
      hoje:    'Nenhum posto vence hoje.',
      semana:  'Nenhum posto vence esta semana.',
      pago:    'Nenhum posto pagou a mensalidade este mês ainda.',
      sempag:  'Todos os postos têm mensalidade configurada.',
    };
    list.innerHTML='<div class="empty-state"><i class="fas fa-filter"></i><p>'+( msgs[filtroPostosAtivo]||'Nenhum resultado.' )+'</p></div>';
